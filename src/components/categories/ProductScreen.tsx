...
import { ProductVariant, useGetVendorInventory, VendorProduct } from "../../api/hooks/vendorInventory";
...
const MenuItem = ({ item, onAddPress }: { item: any, onAddPress: () => void }) => {
    if (!item) return null;

    // Extract properties with fallbacks for nested or flat structure
    ...
    } else if (item.price?.d?.[0]) {
        displayPrice = item.price.d[0];
    } else if (item.product?.sellingPrice?.d?.[0]) {
        displayPrice = item.product.sellingPrice.d[0];
    }
...
const AddItemModal = ({ visible, onClose, productData }: AddItemModalProps) => {
    ...
    const name = productData.product?.name || productData.name || "Unknown Item";
    const description = productData.product?.description || productData.description || "";
    const isVeg = productData.product?.isVeg ?? productData.isVeg ?? true;

    let basePrice = 0;
    if (selectedVariant) {
        basePrice = selectedVariant.price?.d?.[0] || 0;
    } else if (typeof productData.price === "string" || typeof productData.price === "number") {
        basePrice = Number(productData.price);
    } else if (productData.price?.d?.[0]) {
        basePrice = productData.price.d[0];
    } else if (productData.product?.sellingPrice?.d?.[0]) {
        basePrice = productData.product.sellingPrice.d[0];
    }

    const totalPrice = basePrice * itemCount;

    let imageUrl = "https://via.placeholder.com/150";
    if (productData.product?.images?.[0]?.image?.url) {
        imageUrl = productData.product.images[0].image.url;
                        ...
                        <View style={styles.popupHeader}>
                            <View style={styles.popupHeaderImgWrapper}>
                                ...
                                <View style={styles.popupVegBadge}>
                                    <View style={[styles.vegIconDot, { backgroundColor: isVeg ? COLORS.highlight : 'red' }]} />
                                </View>
                            </View>
                            <View style={styles.popupHeaderInfo}>
                                <Text style={styles.popupTitle}>{name}</Text>
                                <Text style={{ fontSize: 12, color: COLORS.textSecondary }} numberOfLines={1}>{description}</Text>
                            </View>
                        </View>
                        <View style={styles.dividerLight} />
...
export default function PizzaProjectScreen({ navigation, route }: { navigation: any; route: any }) {
    ...
    const flatProducts = useMemo(() => {
        ...
        const products = vendorData.pages.flatMap(page => page.products);
        // Modified: Accept items that have either a nested product OR top-level properties (like name)
        return products.filter(item => item && (item.product || item.name));
    }, [vendorData]);
    ...
    const filteredProducts = useMemo(() => {
        ...
        const getPrice = (item: any) => {
            if (typeof item.price === "string" || typeof item.price === "number") return Number(item.price);
            if (item.price?.d?.[0]) return item.price.d[0];
            if (item.product?.sellingPrice?.d?.[0]) return item.product.sellingPrice.d[0];
            return 0;
        };

        switch (activeFilter) {
            case "Veg": return result.filter((item) => (item.product?.isVeg ?? item.isVeg) === true);
            case "Egg": return result.filter((item) => (item.product?.isVeg ?? item.isVeg) === false);
            case "Non-veg": return result.filter((item) => (item.product?.isVeg ?? item.isVeg) === false);
            case "Price - Low to High": return [...result].sort((a, b) => getPrice(a) - getPrice(b));
            case "Price - High to Low": return [...result].sort((a, b) => getPrice(b) - getPrice(a));
            case "Highly Reordered": return result;
    ...
    return (
        <View style={styles.container}>
            ...
            <Animated.ScrollView
                ...
                contentContainerStyle={{ paddingBottom: 150, paddingTop: HEADER_HEIGHT }}
                style={{ zIndex: 1 }}
            >
            ...
            <Animated.View style={[styles.heroContainer, { transform: [{ translateY: headerTranslateY }, { scale: imageScale }], zIndex: 0 }]}>
                <Image source={{ uri: vendorImage || "https://via.placeholder.com/800x400?text=Restaurant" }} style={styles.heroImage} />
                <View style={styles.heroOverlay} />
            </Animated.View>

            <Animated.View pointerEvents="box-none" style={[styles.stickyHeader, { opacity: headerOpacity, zIndex: 10 }]}>
            ...
            <TouchableOpacity style={[styles.floatingBackBtn, { zIndex: 100 }]} onPress={() => navigation?.goBack()}>
                <Ionicons name="arrow-back" size={22} color={COLORS.textPrimary} />
            </TouchableOpacity>
